function Add-VR {
    ...
    if ($Seggregation -eq $true -and $CheckVR -notcontains "$($CountryCode)Landlines") {
        try {
            New-CsOnlineVoiceRoute -Identity "$($CountryCode)Landlines" -NumberPattern "^\$($Prefix)$($FDL)(\d{$($MinLL),$($MaxLL)})$" -OnlinePstnGatewayList $SBC -Priority 1 -OnlinePstnUsages "$($CountryCode)LandlinesOnly" -ErrorAction Stop | Out-Null
    ...
    if ($Seggregation -eq $true -and $CheckVR -notcontains "$($CountryCode)Mobiles") {
        try {
            New-CsOnlineVoiceRoute -Identity "$($CountryCode)Mobiles" -NumberPattern "^\$($Prefix)$($FDM)(\d{$($MinML),$($MaxML)})$" -OnlinePstnGatewayList $SBC -Priority 1 -OnlinePstnUsages "$($CountryCode)MobilesOnly" -ErrorAction Stop| Out-Null
    ...
    if ($Seggregation -eq $false -and $CheckVR -notcontains "$($CountryCode)") {
        try {
            New-CsOnlineVoiceRoute -Identity "$($CountryCode)" -NumberPattern "^\$($Prefix)(\d{$($MinLL),$($MaxML)})$" -OnlinePstnGatewayList $SBC -Priority 1 -OnlinePstnUsages "$($CountryCode)" -ErrorAction Stop | Out-Null
    ...
    if ($International -eq $true -and $CheckVR -notcontains "$($CountryCode)toInternational") {
        try {
            New-CsOnlineVoiceRoute -Identity "$($CountryCode)toInternational" -NumberPattern "^\+(\d{7})+" -OnlinePstnGatewayList $SBC -Priority 1 -OnlinePstnUsages "$($CountryCode)toInternational" -ErrorAction Stop | Out-Null
